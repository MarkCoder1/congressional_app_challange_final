...
export function OverdueTasks({ overdue }: OverdueTasksProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-error-tint border border-error/20 rounded-2xl p-6"
    >
      <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-error">
        ...
        {overdue.tasks.slice(0, 5).map((task, idx) => {
          ...
          return (
            ...
            >
              <Link
                href={`/task/${task.id}`}
                className="flex items-center justify-between bg-white rounded-xl p-4 hover:bg-error-tint transition-all"
              >
                <div>
